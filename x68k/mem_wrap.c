 ...
 * write function
 */
void FASTCALL dma_writemem24(DWORD addr, BYTE val) {
...
void FASTCALL dma_writemem24_word(DWORD addr, WORD val) {
...
void FASTCALL dma_writemem24_dword(DWORD addr, DWORD val) {
...
void FASTCALL cpu_writemem24(DWORD addr, BYTE val) {
    if (addr == WINDRV_PORT) {
        printf("*** TODO: b write %01x to E9F000\n", val);
		return Windrv_ProcessCommand(val);
    }
...
void FASTCALL cpu_writemem24_word(DWORD addr, WORD val) {
    if (addr == WINDRV_PORT) {
        printf("*** TODO: w write %02x to E9F000\n", val);
    }
...
void FASTCALL cpu_writemem24_dword(DWORD addr, DWORD val) {
    if (addr == WINDRV_PORT) {
        printf("*** TODO: dw write %04x to E9F000\n", val);
    }
...
static void FASTCALL wm_main(DWORD addr, BYTE val) {

    if ((BusErrFlag & 7) == 0)
        wm_cnt(addr, val);
}

static void FASTCALL wm_cnt(DWORD addr, BYTE val) {
 ...
 * read function
 */
BYTE FASTCALL dma_readmem24(DWORD addr) { return rm_main(addr); }

WORD FASTCALL dma_readmem24_word(DWORD addr) {
    WORD v;
...
DWORD FASTCALL dma_readmem24_dword(DWORD addr) {
    DWORD v;
...
BYTE FASTCALL cpu_readmem24(DWORD addr) {
    BYTE v;

    if (addr == WINDRV_PORT) {
        printf("*** TODO: b call to E9F000\n");
		// TODO: move first_read to a global variable, and set it to true in Windrv_Init
        static char first_read = 1;
        if (first_read) {
            first_read = 0;
            return 'W'; // EX68 の WINDRV は 'W' を期待している
        }
        return Windrv_ReadCommand();
    }
...
WORD FASTCALL cpu_readmem24_word(DWORD addr) {
    WORD v;

    if (addr == WINDRV_PORT) {
        printf("*** TODO: w call to E9F000\n");
    }
...
DWORD FASTCALL cpu_readmem24_dword(DWORD addr) {
    DWORD v;

    if (addr == WINDRV_PORT) {
        printf("*** TODO: dw call to E9F000\n");
    }
...
static BYTE FASTCALL rm_font(DWORD addr) { return FONT[addr & 0xfffff]; }

static BYTE FASTCALL rm_ipl(DWORD addr) { return IPL[(addr & 0x3ffff) ^ 1]; }
...
void FASTCALL cpu_setOPbase24(DWORD addr) {
