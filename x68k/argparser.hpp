...
class argmap {
  ...
  public:
    ...
    bool parsed_successfully() const noexcept { return success; }
};
...
class parser {
  private:
    struct argstruct {
        ...
        argstruct(const std::string& sa, const std::string& la,
                  const std::string& hs, bool bt, bool rq, bool ps)
            : shortarg(sa), longarg(la), helpstr(hs), booltype(bt),
              required(rq), parsed(ps) {}
    };
  ...
  private:
    void remove_equals(std::vector<std::string>& argv) const {
        int new_argc =
            std::count_if(argv.begin(), argv.end(),
                          [](const std::string& s) {
    ...
    void print_help_string() const {
        ...
        for (const auto& as : m_args) {
            ...
            if (!sa.empty()) {
                if (!la.empty()) {
                    fprintf(stdout, "%s%s,%s%s ", lbrak, sa.c_str(), la.c_str(),
                            rbrak);
                } else {
        ...
        for (const auto& as : m_args) {
            ...
            int arg_len = (shortlen && longlen) ? (shortlen + 2 + longlen)
                                                : (shortlen + longlen);
            if (arg_len > max_len)
                max_len = arg_len;
        }
        ...
        for (const auto& as : m_args) {
            ...
            if (!sa.empty()) {
                if (!la.empty()) {
                    fprintf(stdout, "%s%s, %s", leftpadstr, sa.c_str(),
                            la.c_str());
                } else {
  ...
  public:
    ...
    bool add(const std::string& shortarg, const std::string& longarg,
             const std::string& helpstr, mode m = mode::OPTIONAL) {
        ...
        if (!shortarg.empty() && (shortarg.size() != 2 || shortarg[0] != '-' ||
                                  shortarg[1] == '-')) {
            m_any_adds_failed = true;
            return false;
        }
        if (!longarg.empty() &&
            (longarg.size() <= 2 || longarg[0] != '-' || longarg[1] != '-')) {
            m_any_adds_failed = true;
            return false;
        }
        ...
        if (std::count_if(m_args.begin(), m_args.end(), has_duplicate_args) >
            0) {
            m_any_adds_failed = true;
            return false;
        }
        ...
        m_args.emplace_back(shortarg, longarg, helpstr, booltype, required,
                            false);
        ...
        if (m_any_adds_failed) {
            ...
            if (std::any_of(m_argv.begin(), m_argv.end(), is_rogue_equal)) {
                ...
                if (success) {
                    ...
                    auto is_unparsed = [](const argstruct& as) {
                        return as.required && !as.parsed;
                    };
                    if (std::any_of(m_args.begin(), m_args.end(),
                                    is_unparsed)) {
                        success = false;
                    }
                }
    ...
    const std::vector<std::string>& argv() const noexcept { return m_argv; }
};
} // namespace ap
